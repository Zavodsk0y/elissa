          ...
          <div class="grid_parts">
            <div v-for="part in parts" :key="part.id" class="serviceContainer">
              <figure class="partInfo">
                ...
                <div class="cartQuantityControls d-f j-c-c a-i-c">
                  <input type="number" v-model.number="cartItems[part.id].quantity" @input="inputQuantity(part)" class="quantityInput fs-17px fw-400 ml-2">
...
export default {
  data() {
    return {
      parts: [],
      cartItems: {}, // объект для хранения количества товаров в корзине
      currentPage: 1,
      totalPages: 0,
    };
  },
  ...
  methods: {
      ...
      axios.get(`http://localhost/api/parts?page=${page}&limit=6`, {
        headers: {
          ...
          .then(response => {
            ...
            // Инициализируем cartItems для всех товаров
            this.parts.forEach(part => {
              this.$set(this.cartItems, part.id, { quantity: 0 });
            });
          })
          .catch(error => {
    ...
    inputQuantity(part) {
      // Дополнительная проверка на ввод с клавиатуры
      if (this.cartItems[part.id].quantity < 1) {
        this.cartItems[part.id].quantity = 1;
      } else if (this.cartItems[part.id].quantity > 10) {
